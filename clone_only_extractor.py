...
# Enforce UTF-8 for subprocess and file IO robustness
...
# Load GitHub tokens (optional, for higher rate limits)
...
# === PATHS (cross-platform, env-overridable) ===
# You can override via env: BASE_DIR, CSV_PATH
base_dir_env = os.getenv("BASE_DIR")
if base_dir_env:
	base_dir = Path(base_dir_env)
else:
	base_dir = Path.cwd() / "android_repo_extract"

csv_path_env = os.getenv("CSV_PATH")
if csv_path_env:
	csv_path = Path(csv_path_env)
else:
	csv_path = base_dir / "URL_List_Rand.csv"
...
def run(cmd: list[str], cwd: str | Path | None = None, check: bool = True) -> subprocess.CompletedProcess:
	"""Run a command and return the completed process with UTF-8 text stdout.
	Raises CalledProcessError if check is True and the command fails.
	"""
	return subprocess.run(
		cmd,
		cwd=str(cwd) if cwd is not None else None,
		stdout=subprocess.PIPE,
		stderr=subprocess.STDOUT,  # single stream
		text=True,
		encoding="utf-8",
		errors="replace",
		check=check,
...
if not csv_path.exists():
	raise SystemExit(f"❌ CSV file not found: {csv_path}. Set CSV_PATH env or place file at this path.")

df = pd.read_csv(csv_path)
df.columns = df.columns.str.strip().str.lower()
if "github_url" not in df.columns:
	raise SystemExit("❌ Input CSV must contain a 'github_url' column.")
...
# ========= Sampling =========
if SAMPLE_LIST_RAW:
	sample_indices_to_keep = set(map(int, filter(None, SAMPLE_LIST_RAW.split(","))))
...
for i in range(START_NUMBER - 1, min(len(df), MAX_PROJECTS)):
	url = str(df.iloc[i]["github_url"]).strip()
	...
	try:
		...
		review_status_rows.append({"html_url": url, "clone_status": "no", "yml_detected": "no"})
		...
		fail_row = {"repo_index": repo_index, "repo_name": repo_name, "github_url": url, "error_message": error_message}
	...
	# --- Optional: extract commit metadata (kept) ---
	if local_commit_count > 0:
		try:
			...
			for commit in commit_hashes:
				cp = run(
					["git", "-C", str(repo_path), "show", "--quiet", f"--pretty=format:%H|%an|%ae|%ad|%s", "--date=iso", commit],
					check=False,
				)
	...
	review_status_row = {"html_url": url, "clone_status": "yes", "yml_detected": "yes" if any_yml else "no"}
	...
	try:
		...
		if TOKENS:
			headers = {"Authorization": f"token {TOKENS[token_index % len(TOKENS)]}", "Accept": "application/vnd.github+json"}
			token_index += 1
		...
		# Save contributor names (first page)
		if TOKENS:
			headers = {"Authorization": f"token {TOKENS[token_index % len(TOKENS)]}", "Accept": "application/vnd.github+json"}
			token_index += 1
		...
		if r_contrib.status_code == 200:
			try:
				contributor_logins = [c["login"] for c in r_contrib.json() if isinstance(c, dict) and "login" in c]
	...
	# === Move to Cloned_Sample or delete (disabled by default) ===
	# try:
