...
# ====== PATTERNS ======
RE_TRIG_INSTRU_GRADLE = re.compile(
    r"\bgradle(?:w(?:\.bat)?)?\b[^\n]*\b(?::\S+:)?(connected(?:AndroidTest|Check)|allDevices(?:Test|Check)|\w+AndroidTest)\b",
    re.IGNORECASE
)
RE_TRIG_UNIT_GRADLE = re.compile(
    r"\bgradle(?:w(?:\.bat)?)?\b[^\n]*\b(:\S+:)?(test(?:\w*UnitTest)?|check)\b",
    re.IGNORECASE
)
# Extract :module or :module:submodule from a Gradle command line
RE_TRIG_GRADLE_MODULE = re.compile(
